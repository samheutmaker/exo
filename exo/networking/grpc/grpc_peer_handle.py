...
class GRPCPeerHandle(PeerHandle):
    ...
    async def connect(self):
        self.channel = grpc.aio.insecure_channel(
            self.address, options=[("grpc.max_metadata_size", 32 * 1024 * 1024)]
        )
    ...
    async def get_inference_result(
        ...
        return np.frombuffer(
            response.tensor.tensor_data, dtype=np.dtype(response.tensor.dtype)
        ).reshape(response.tensor.shape), response.is_finished

    async def reset_shard(self, shard: Shard) -> None:
        request = node_service_pb2.ResetShardRequest(
            shard=node_service_pb2.Shard(
                model_id=shard.model_id,
                start_layer=shard.start_layer,
                end_layer=shard.end_layer,
    ...
    async def global_reset(
        self, base_shard: Shard, visited: set[str], max_depth: int
    ) -> None:
        request = node_service_pb2.GlobalResetRequest(
            base_shard=node_service_pb2.Shard(
    ...
    async def send_result(
        ...
        request = node_service_pb2.SendResultRequest(
            request_id=request_id, result=result, is_finished=is_finished
        )
