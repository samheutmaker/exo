...
shard_mappings = {
    "llama-3-8b": {
        ...
        "TinygradDynamicShardInferenceEngine": Shard(
            model_id="llama3-8b-sfr", start_layer=0, end_layer=0, n_layers=32
        ),
    },
    "llama-3-70b": {
        "MLXDynamicShardInferenceEngine": Shard(
...
def resolve_tokenizer(model_id: str):
    try:
        if DEBUG >= 2:
            print(f"Trying AutoTokenizer for {model_id}")
        ...
        if DEBUG >= 2:
            print(
                f"Failed to load tokenizer for {model_id}. Falling back to tinygrad tokenizer"
            )

    try:
        if DEBUG >= 2:
            print(f"Trying tinygrad tokenizer for {model_id}")
        ...
        if DEBUG >= 2:
            print(
                f"Failed again to load tokenizer for {model_id}. Falling back to mlx tokenizer"
            )

    if DEBUG >= 2:
        print(f"Trying mlx tokenizer for {model_id}")
...
class ChatGPTAPI:
    def __init__(
        ...
        self.static_dir = (
            Path(__file__).parent.parent.parent / "tinychat/examples/tinychat"
        )
    ...
    async def handle_post_chat_token_encode(self, request):
        ...
        shard = shard_mappings.get(data.get("model", "llama-3-8b"), {}).get(
    ...
    async def handle_post_chat_completions(self, request):
        ...
        chat_request = ChatCompletionRequest(
            data.get("model", "llama-3-8b"), messages, data.get("temperature", 0.0)
        )
        if (
            chat_request.model and chat_request.model.startswith("gpt-")
        ):  # to be compatible with ChatGPT tools, point all gpt- model requests to llama instead
            chat_request.model = "llama-3-8b"
        shard = shard_mappings.get(chat_request.model, {}).get(
            self.inference_engine_classname
        )
        if not shard:
            return web.json_response(
                {
                    "detail": f"Invalid model: {chat_request.model}. Supported: {list(shard_mappings.keys())}"
                },
                status=400,
            )
        ...
        try:
            if DEBUG >= 2:
                print(
                    f"Waiting for response to finish. timeout={self.response_timeout_secs}s"
                )
            ...
            if stream:
                ...
                _, tokens, _ = await callback.wait(
                    on_result, timeout=self.response_timeout_secs
                )
                if (
                    ...
                    if DEBUG >= 2:
                        print(
                            f"Pending stream task. Waiting for stream task to complete."
                        )
                    try:
    ...
    async def run(self, host: str = "0.0.0.0", port: int = 8000):
        ...
        if DEBUG >= 0:
            ...
            print(
                f"ChatGPT API endpoint served at {terminal_link(f'http://localhost:{port}/v1/chat/completions')}"
            )
