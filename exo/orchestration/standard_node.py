...
class StandardNode(Node):
    def __init__(
        ...
        self.topology_viz = (
            TopologyViz(
                chatgpt_api_endpoint=chatgpt_api_endpoint, web_chat_url=web_chat_url
            )
            if not disable_tui
            else None
        )
    ...
    async def _process_prompt(
        ...
        result, inference_state, is_finished = await self.inference_engine.infer_prompt(
            shard, prompt, inference_state=inference_state
        )
        is_finished = (
            is_finished
    ...
    async def _process_tensor(
        ...
        try:
            ...
            (
                ...
                shard, tensor, inference_state=inference_state
            )
            is_finished = (
                is_finished
                or len(self.buffered_token_output[request_id][0])
    ...
    async def reset_shard(self, base_shard: Shard) -> None:
        ...
        if DEBUG >= 2:
            print(f"Resetting shard: {base_shard}")
    ...
    async def update_peers(self, wait_for_peers: int = 0) -> None:
        ...
        for peer in self.peers:
            ...
            if not is_connected:
                ...
                if DEBUG >= 0:
                    print(
                        f"Connected to peer {peer.device_capabilities()} ({peer.id()=})"
                    )
    ...
    async def collect_topology(
        ...
        for peer in self.peers:
            ...
            if peer.id() in prev_visited:
                if DEBUG >= 2:
                    print(f"Already visited {peer.id()}. Skipping...")
                continue

            if max_depth <= 0:
                if DEBUG >= 2:
                    print(f"Max depth reached. Skipping...")
                continue

            try:
                other_topology = await peer.collect_topology(
                    visited, max_depth=max_depth - 1
                )
                if DEBUG >= 2:
    ...
    async def global_reset(
        self, base_shard: Shard, visited: set[str] = set(), max_depth: int = 2
    ) -> None:
        shard = self.get_current_shard(base_shard)
        await self.reset_shard(shard)

        if DEBUG >= 2:
            print(f"Global reset {base_shard=} {max_depth=} {visited=}")
