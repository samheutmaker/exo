...
parser.add_argument(
    "--chatgpt-api-response-timeout-secs",
    type=int,
    default=90,
...
discovery = GRPCDiscovery(
    args.node_id, args.node_port, args.listen_port, args.broadcast_port
)
node = StandardNode(
    args.node_id,
    None,
...
node.on_token.register("main_log").on_next(
    lambda _, tokens, __: print(
        inference_engine.tokenizer.decode(tokens)
        if hasattr(inference_engine, "tokenizer")
        else tokens
    )
)
